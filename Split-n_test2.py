...
def send(ser, cmd):
    ...
    #print("Serving Cell", ser_cell)
    if gps_loc is True:
        elements = output.split(':')[1].split(',')
        #print(elements)
        keys = ['UTC Time:','Latitude:', 'Longitude:', 'HDOP:', 'Altitude:','Fix:', 'Course:', 'Speed (km):','Speed (kn):','date:','Satellites:']
        my_dict = dict(zip(keys,elements))
        print(my_dict.get('state'))
        for key, value in my_dict.items():
            print(key,value)
            if (key,value) != "":
                loginfo = str(datetime.datetime.utcnow())+ ":"+ str(output)
                logger.info(loginfo)
    
    if ser_cell is True:
        elements = output.split(':')[1].split(',')
        #print(elements)
        keys = ['Topic:','Connection State:', 'Acess_Tech:', 'FDD/TDD:', 'MCC:','MNC:', 'CellID:', 'PCID:','EARFCN:','FBI:','UL_BW:','DL_BW:','TAC:','RSRP:','RSCP:','RSRQ:','RSSI:','SINR:','CQI:','TX-Power:','SRxlev:']
        my_dict = dict(zip(keys,elements))
        print(my_dict.get('state'))
        for key, value in my_dict.items():
            print(key,value)
    ...
    if neg_info_inter is True:
        elements = output.split(':')[1].split(',')
        #print(elements)
        keys = ['Topic:','Acess_Tech:','EARFCN','PCID:', 'RSRQ:','RSRP:', 'RSSI:', 'SINR:','srxlev:','Cell_resel_priority:','threshold_Xlow:','threshold_Xhigh:']
        my_dict = dict(zip(keys,elements))
        print(my_dict.get('state'))
        for key, value in my_dict.items():
            print(key,value)
    ...
    if neg_info_intra is True:
        elements = output.split(':')[1].split(',')
        #print(elements)
        keys = ['Topic:','Acess_Tech:','EARFCN','PCID:', 'RSRQ:','RSRP:', 'RSSI:', 'SINR:','srxlev:','Cell_resel_priority:', 's_non_intra_search','thresh_serv_low:','s_intra_search:']
        my_dict = dict(zip(keys,elements))
        print(my_dict.get('state'))
        for key, value in my_dict.items():
            print(key,value)
    ...
    #for i in range(len(modem_log)):
        #if len(modem_log[i])> 1:
            #pass
          #print(modem_log[i])
    #length = len(modem_log)
    #print(length)
    #print("Logs ->",modem_log[0])
